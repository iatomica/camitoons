...
import { User, BookOpen, Heart, Smile, Users } from 'lucide-react';
import { LUNA_IMAGES } from '../data/lunaImages';

interface AboutSectionProps {
  darkMode: boolean;
  // Kept blocks and refresh props empty to avoid breaking App.tsx callers, but unused
  blocks?: any;
...
export const AboutSection: React.FC<AboutSectionProps> = ({ 
  ...
  const bioText = "Me llamo **Camila Maestrojuan**, soy docente de Educación Inicial. A partir de mi experiencia en instituciones y del vínculo cotidiano con infancias, nace el deseo de escribir literatura infantil.\n\nCon el motivo de celebrar la ternura de los primeros años de vida, se creó la colección **\"Luna está creciendo\"**, en donde se disfrutan historias construidas con amor y con una profunda intención pedagógica.\n\nSu escritura pone foco en la educación emocional, en los vínculos y la curiosidad por el entorno, con un lenguaje cercano y una mirada respetuosa de las etapas evolutivas de cada niño/a.";

  const renderParagraph = (text: string) => {
    const parts = text.split(/\*\*([^*]+)\*\*/g);
    return parts.map((part, index) => {
      if (index % 2 === 1) {
        return <strong key={index} className="text-sky-900 dark:text-sky-300 font-black">{part}</strong>;
      }
      return part;
    });
  };
  ...
  return (
    <section id="sobre-mi" className="py-12 lg:py-20 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        ...
        <div className="relative max-w-4xl mx-auto rounded-3xl border-2 border-sky-300/80 dark:border-sky-800/60 bg-gradient-to-br from-sky-50 via-white to-sky-100/50 dark:from-slate-900 dark:via-slate-900 dark:to-sky-950/40 shadow-xl p-6 sm:p-8 text-slate-900 dark:text-slate-100 overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
            ...
            <div className="lg:col-span-4 flex flex-col items-center text-center space-y-2.5">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] font-extrabold text-sky-800 dark:text-sky-300">
            ...
            <div className="lg:col-span-8 space-y-3.5 text-center md:text-left">
              ...
              <div className="inline-flex items-center space-x-1.5 px-3 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider text-sky-900 dark:text-sky-300 bg-sky-100 dark:bg-sky-950/80 border border-sky-300 dark:border-sky-800 shadow-sm">
                <User className="w-3 h-3 text-sky-700 dark:text-sky-400" />
                <span>Biografía de la Autora</span>
              </div>

              <div className="space-y-2.5 text-xs sm:text-sm leading-relaxed text-slate-900 dark:text-slate-200 font-bold">
              ...
              <div className="pt-1">
                <div className="flex flex-wrap justify-center md:justify-start gap-1.5">
                  {[
                    ...
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg text-[11px] font-bold border bg-white/90 dark:bg-slate-800/90 text-sky-955 dark:text-sky-200 border-sky-300/70 dark:border-sky-800 shadow-sm"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className={`p-8 sm:p-12 rounded-3xl border shadow-2xl space-y-10 transition-all ${
          darkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-gradient-to-br from-purple-50/70 via-white to-pink-50/70 border-purple-200'
        }`}>
          <div className="text-center max-w-3xl mx-auto space-y-3">
            <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider text-pink-850 dark:text-pink-300 bg-pink-100/80 dark:bg-pink-950/60 border border-pink-200 dark:border-pink-800">
              <BookOpen className="w-3.5 h-3.5" />
              <span>Presentación del Proyecto</span>
            </div>
            <p className="text-lg font-bold text-purple-850 dark:text-purple-400 italic">
              Historias para crecer
            </p>
            <p className={`text-sm sm:text-base leading-relaxed font-bold ${darkMode ? 'text-slate-200' : 'text-slate-900'}`}>
            ...
            <div className="p-2.5 sm:p-3 bg-white dark:bg-slate-900 border-2 border-sky-300 dark:border-sky-700 rounded-3xl shadow-md hover:shadow-lg transition-all">
              <div className="p-5 rounded-2xl bg-sky-50/50 dark:bg-slate-950/60 border border-sky-200 dark:border-sky-800/60 space-y-3 h-full flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <p className={`text-xs leading-relaxed ${darkMode ? 'text-slate-300 font-semibold' : 'text-slate-950 font-bold'}`}>
            ...
            <div className="p-2.5 sm:p-3 bg-white dark:bg-slate-900 border-2 border-sky-300 dark:border-sky-700 rounded-3xl shadow-md hover:shadow-lg transition-all">
              <div className="p-5 rounded-2xl bg-sky-50/50 dark:bg-slate-950/60 border border-sky-200 dark:border-sky-800/60 space-y-3 h-full flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <h3 className="font-extrabold text-base text-slate-950 dark:text-white">Mirada Pedagógica</h3>
                  <p className={`text-xs leading-relaxed ${darkMode ? 'text-slate-300 font-semibold' : 'text-slate-950 font-bold'}`}>
            ...
            <div className="p-2.5 sm:p-3 bg-white dark:bg-slate-900 border-2 border-sky-300 dark:border-sky-700 rounded-3xl shadow-md hover:shadow-lg transition-all">
              <div className="p-5 rounded-2xl bg-sky-50/50 dark:bg-slate-950/60 border border-sky-200 dark:border-sky-800/60 space-y-3 h-full flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <h3 className="font-extrabold text-base text-slate-950 dark:text-white">Puente Intergeneracional</h3>
                  <p className={`text-xs leading-relaxed ${darkMode ? 'text-slate-300 font-semibold' : 'text-slate-950 font-bold'}`}>
